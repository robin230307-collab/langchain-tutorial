      ...
      tutorials: [
        {
          ...
          aiIntegration: {
            ...
            basicCode: `from langchain.prompts import PromptTemplate
...
# 填入变量
prompt = template.format(style="专业学术", topic="人工智能")

# 连接 OpenAI
llm = OpenAI(api_key="你的API-KEY")
response = llm.invoke(prompt)
print(response)`,
            supportedAIs: ['OpenAI (GPT-4, GPT-3.5)', 'Claude', 'Gemini', '通义千问', '文心一言'],
            notes: [
              'API Key 需要妥善保管，不要暴露在前端代码中',
              '不同 AI 的响应格式可能略有差异',
              '建议设置 temperature 参数控制随机性'
            ]
          },
          quickStart: {
            title: '5分钟快速开始',
...
\`\`\``
        ...
        },
        {
          ...
          aiIntegration: {
            installCommand: 'pip install langchain langchain-openai',
            apiKeyUrl: 'https://platform.openai.com/api-keys',
            basicCode: `from langchain.prompts import FewShotPromptTemplate
from langchain_openai import OpenAI
...
# 连接 AI
llm = OpenAI(api_key="你的API-KEY")
response = llm.invoke(prompt.format(adjective="前进"))
print(response)`,
            supportedAIs: ['OpenAI (GPT-4, GPT-3.5)', 'Claude', 'Gemini', '通义千问', '文心一言'],
            notes: [
              '示例数量建议3-5个，过多会影响响应速度',
              '示例应该覆盖边界情况和常见模式',
              '示例顺序可能会影响结果，可以尝试调整'
            ]
          },
...
4. **避免歧义** - 示例要清晰无歧义`
        ...
        },
        {
          ...
          aiIntegration: {
            installCommand: 'pip install langchain langchain-openai',
            apiKeyUrl: 'https://platform.openai.com/api-keys',
            basicCode: `from langchain.output_parsers import JsonOutputParser
from langchain_openai import OpenAI
...
# 调用
result = chain.invoke({"text": "张三，28岁，擅长Python开发"})
print(result)  # 输出: {'姓名': '张三', '年龄': 28, '技能': ['Python']}`,
            supportedAIs: ['OpenAI (GPT-4, GPT-3.5)', 'Claude', 'Gemini', '通义千问', '文心一言'],
            notes: [
              '使用 Pydantic 可以定义更复杂的输出格式',
              'OutputParser 会自动处理格式错误并重试',
              '可以结合 Zod 进行类型验证'
            ]
          },
          quickStart: {
