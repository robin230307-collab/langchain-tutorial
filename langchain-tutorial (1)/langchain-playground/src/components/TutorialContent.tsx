import { ArrowLeft, ChevronDown, ChevronUp, Copy, Check, BookOpen, Target, Lightbulb, Code, Layers, HelpCircle, Cpu, ArrowRight } from 'lucide-react'
...
export function TutorialContent({ tutorial, onBack }: TutorialContentProps) {
      ...
      <div className="mt-6 md:mt-8 p-4 md:p-6 bg-slate-50 rounded-2xl">
        ...
        <div className="flex flex-wrap gap-2">
          {tutorial.relatedTools.map((tool, idx) => (
            <button
              key={idx}
              className="px-3 md:px-4 py-1.5 md:py-2 bg-white border border-slate-200 rounded-lg text-xs md:text-sm text-slate-700 hover:border-green-300 hover:text-green-600 transition-colors flex items-center gap-1.5 md:gap-2"
            >
